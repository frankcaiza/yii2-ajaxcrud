...
namespace frankcaiza\ajaxcrud\generators;
...
class Generator extends \yii\gii\Generator {
    ...
    public function getDescription() {
        return 'Este vendor es personalizado para desarrollo de ELIGOS';
    }
    ...
    function requireKey($needle, array $array) {
        ...
        foreach ($array as $key => $value) {
            ...
            if (is_array($value)) {
                if ($x = $this->requireKey($needle, $value)) {
                    return $x;
                }
            }
        }
    ...
    public function generateActiveField($attribute) {
        ...
        if ($attribute == 'estado') {
            return "echo \$form->field(\$model, '$attribute')->dropDownList(ArrayHelper::map(\common\models\GeneralCatalogo::listarCatalogo('idCatalogo') , 'value', 'nombre'))";
        }
        $column = $tableSchema->columns[$attribute];

        if ($tableSchema->foreignKeys) {
            
            if ($this->requireKey($attribute, $tableSchema->foreignKeys)) {
                
                return "  \$lista=ArrayHelper::map('cambiar-a-modelo-relacional' , 'id', 'nombre-campo');\n"
        ...
        if ($column->phpType === 'boolean' || $column->phpType === 'bit') {
            return "echo \$form->field(\$model, '$attribute')->checkbox()";
        } elseif ($column->type === 'text') {
            return "echo \$form->field(\$model, '$attribute')->textarea(['rows' => 2])";
        } else {
